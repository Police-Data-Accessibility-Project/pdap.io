  ...
  <main class="max-w-[1800px] mx-auto px-4 md:px-8 py-8 grid gap-8">
    <section class="flex flex-col gap-4">
      ...
      >
        <div v-if="!isFollowedNational">
          ...
          >
            <span v-if="!followNationalPending">
              {{ followLabels.buttonLabel }}
            </span>
            <span v-else>{{ followLabels.pendingLabel }}</span>
          </Button>
          <p
          ...
          >
            ...
            {{ followLabels.signedOutPrompt }}
          </p>
        </div>
        <p v-else class="text-med text-wineneutral-500">
          {{ followLabels.followingMessage }}
        </p>
      </div>
    </section>
      ...
      <div class="relative w-full">
        <div class="h-0 md:h-[70vh] w-full">
          <DataSourceMap
            ...
            v-bind="mapBindings"
            class="h-full w-full"
            :sidebar-component="mapSidebarComponent"
            :sidebar-props="sidebarExtraProps"
            :show-locality-markers="mapOptions.showLocalityMarkers"
            :use-dynamic-breakpoints="mapOptions.useDynamicBreakpoints"
            :show-sidebar="showSidebar"
            :hide-sidebar-without-selection="
              mapOptions.hideSidebarWithoutSelection
            "
    ...
    <section class="grid gap-4">
      <header class="flex flex-col gap-1">
        <h2 class="text-2xl font-semibold">{{ resultsHeading }}</h2>
...
const props = defineProps({
  ...
  mapOptions: {
    ...
    default: () => ({
      ...
      hideSidebarWithoutSelection: false
    })
  },
  resultsHeading: {
    type: String,
    default: 'Filtered Data Sources'
  },
  resultsDescription: {
    type: String,
    default:
      'Results are grouped by geography and agency. Scroll or use the map to focus on a specific area.'
  },
  followCopy: {
    type: Object,
    default: () => ({})
  ...
  geographicLevels: {
    type: Array,
    default: () => ALL_LOCATION_TYPES.filter((type) => type !== 'federal')
  }
});
...
const normalizedRecordTypes = computed(() => props.recordTypes ?? []);
const normalizedRecordCategories = computed(() => props.recordCategories ?? []);

const mapSidebarComponent = computed(() => props.mapSidebarComponent ?? null);

const baseParams = computed(() => {
  const params = {
    record_types: normalizedRecordTypes.value.join(',')
  };
  if (normalizedRecordCategories.value.length) {
    params.record_categories = normalizedRecordCategories.value.join(',');
  }
  return params;
});

const followLabels = computed(() => {
  const defaults = {
    buttonLabel: 'Follow this search',
    pendingLabel: 'Following…',
    signedOutPrompt: ' to follow this search.',
    followingMessage:
      "You're following this search. See your profile for updates.",
    successMessage: `Following ${props.title}.`,
    errorMessage: 'Error following this search. Please try again.'
  };
  return { ...defaults, ...(props.followCopy ?? {}) };
});

const mapQuery = useQuery({
  queryKey: [props.portalId, 'map', normalizedRecordTypes],
  queryFn: async () => {
    const response = await getMapLocations(baseParams.value);
...
const resultsQuery = useQuery({
  queryKey: [props.portalId, 'results', normalizedRecordTypes],
  queryFn: async () => {
    const response = await search(baseParams.value);
...
const followNationalMutation = useMutation({
  mutationFn: async () => followNationalSearch(baseParams.value),
  onSuccess: () => {
    ...
    toast.success(followLabels.value.successMessage);
  },
  onError: () => {
    toast.error(followLabels.value.errorMessage);
  }
});
